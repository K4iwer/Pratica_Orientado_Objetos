...
using namespace std;                             //so ta aqui pra testes -> L
...
Modulo* auxiliarLerArquivo(ifstream& input, Modulo* mod) {           //talvez a passagem de parametro esteja errada  // colocando pra analizar o f logo no comeco ficaria melhor
    ...
    while (input && tipoDoCirc != "f" && tipoDoCirc != "") {                        
        if (tipoDoCirc != "S" && tipoDoCirc != "P" && tipoDoCirc != "R" && tipoDoCirc != "A" && tipoDoCirc != "D" && tipoDoCirc != "I") {
            ...
            cout << "Adicionou R"<< endl;
            modRea = (ModuloRealimentado*)auxiliarLerArquivo(input, modRea);              // checar depois pq quando eu fiz ele ainda n tava pronto  
...
Modulo* PersistenciaDeModulo :: lerDeArquivo() {                       //ainda n tá defendendo de formatações erradas
    Modulo* modOut;    //desse jeito ele coloca tudo dentro um do outor, é pra ser assim mesmo? --> modulo é abstrato, n pode ele n pode ser instanciado
    ifstream input;                                                     
...
/*                        //metodo q poderia criar um módulo normal caso o Vitor saiba o problema q tá dando da memória
